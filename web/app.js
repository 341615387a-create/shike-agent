...
function home(){
  ...
  const publicNote=state.config?.demo_mode?'<p class="public-note">GitHub Pages 静态交互演示 · 不调用模型，演示内容只保存在当前浏览器。真实对话能力请运行本地版。</p>':state.config?.public_mode?'<p class="public-note">公开测试版 · 对话会保存在这个浏览器对应的独立空间，并发送给模型服务生成回复。清除浏览器 Cookie 后无法找回原记录。</p>':'';
...
function renderSession(){
  ...
  const demoTag=state.config?.demo_mode&&!state.replay?'<div class="example-tag">静态交互演示 · 回复由浏览器内置规则生成，不代表真实模型效果</div>':'';
