...
本版以「理解持续生长，方向跟随用户」重新实现。用户可以从日常陈述、问题或已有计划出发，不限定创作赛道，不强制最终结论。

公开部署使用仓库根目录的 `render.yaml`，具体操作见 [`DEPLOY-RENDER.md`](DEPLOY-RENDER.md)。公网模式使用签名浏览器 Cookie 隔离每位访客的会话、个人记忆和灵感笔记；模型密钥只从服务器环境变量读取，访客不能打开或修改模型设置，并有基础的每IP小时限流。本地模式继续使用现有单用户数据，不受影响。
